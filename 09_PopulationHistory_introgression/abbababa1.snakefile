...
# the script was original developed by Genís Garcia-Erill

import pandas as pd
import re

ANGSD="/maps/projects/bos/apps/angsd/angsd"
JACKKNIFE="/home/wlk579/1.0JavaBanteng_project/6.4.Analyses.ABBABABA/jackKnife.R"
...
rule do_idlist:
    ...
    run:
        with open(input.bamlist, "r") as fh:
            ...
            #bams = bams[:-1] this only with -useLa2st 1
...
rule do_abbababa_per_chrom:
    ...
    shell: "{ANGSD} -doAbbababa 1 -doCounts 1 -GL 2 -out {params.outprefix}  -bam {input.bamlist} -minmapQ {params.MINMAPQ} -minq {params.MINQ} -r {params.r} -sites {params.regions} -blockSize {params.blocksize} -anc {params.anc} -useLast 1"
